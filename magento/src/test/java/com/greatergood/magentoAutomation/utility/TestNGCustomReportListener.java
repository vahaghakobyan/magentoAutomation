...
public class TestNGCustomReportListener implements IReporter {
	...
	private void resultSummary(ISuite suite, IResultMap tests, String testname, String style, String details) {
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				buff.append("<td><a href='#m" + m_methodIndex + "'>" + qualifiedName(method) + "</a>"
						+ (null == testInstanceName ? "" : "<br>(" + testInstanceName + ")") + "</td>" + "<td>"
						+ (null == description ? "" : description) + "</td>"
	...
	private void startResultSummaryTable(String style) {
		...
		writer.println("<tr><th>Class</th>"
				+ "<th>Method</th><th>Description</th><th>Exception</th><th>Execution Time</th></tr>");
		m_row = 0;
	}
